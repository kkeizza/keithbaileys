...
const makeSocket = (config) => {
    ...
    /**
     * Resolves PN -> LID mappings via a USync query. This is a closure so it
     * can safely reference `query`/`generateMessageTag` even though they are
     * declared further down in this function: by the time this is actually
     * *called* (during message send/receive), those are already initialized.
     * Passed into the LID mapping store so signal sessions stop flip-flopping
     * between PN and LID addresses for the same contact.
     */
    const pnFromLIDUSync = async (jids) => {
        ...
        try {
            ...
            if (result) {
                return result.list.filter(a => !!a.lid).map(({ lid, id }) => ({ pn: id, lid }));
            }
        }
        catch (err) {
            logger?.debug?.({ err }, 'pnFromLIDUSync query failed');
        }
        return [];
    };
    ...
    /** run a USync query (used internally for PN<->LID resolution, also exposed on the socket) */
    ...
    const fetchAccountReachoutTimelock = async () => {
        ...
        const result = {
            isActive: !!(queryResult === null || queryResult === void 0 ? void 0 : queryResult.is_active),
            timeEnforcementEnds: (queryResult === null || queryResult === void 0 ? void 0 : queryResult.time_enforcement_ends) && (queryResult === null || queryResult === void 0 ? void 0 : queryResult.time_enforcement_ends) !== '0'
                ? new Date(parseInt(queryResult.time_enforcement_ends, 10) * 1000)
                : undefined,
            enforcementType: (_a = queryResult === null || queryResult === void 0 ? void 0 : queryResult.enforcement_type) !== null && _a !== void 0 ? _a : Types_1.ReachoutTimelockEnforcementType.DEFAULT
        };
