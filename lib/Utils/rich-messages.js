...
const toTableMetadataV2 = (input) => {
  if (input === undefined || input === null) {
    throw new Error(
      "Table input is missing — pass either { headers: string[], rows: string[][] }, " +
        "a 2D array [[headers...], [row1...], ...], or [title, \"h1|h2\", \"r1c1|r1c2;;r2c1|r2c2\"]",
    );
  }
  ...
  if (!Array.isArray(input) && typeof input === "object") {
    // Documented shape: { title?, headers, rows }
    if (!Array.isArray(input.headers) || input.headers.length === 0) {
      throw new Error("Table input must include a non-empty \"headers\" array");
    }
    title = input.title || "";
    header = input.headers.map(String);
    parsedRows = Array.isArray(input.rows)
    ...
    // 2D array shape: [[headers...], [row1...], [row2...]]
    const [headerRow, ...rows] = input;
    header = headerRow.map(String);
    parsedRows = rows.map((row) => (Array.isArray(row) ? row.map(String) : [String(row)]));
  } else if (Array.isArray(input) && input.length > 0) {
    // Legacy flat shape: [title, "h1|h2", "r1c1|r1c2;;r2c1|r2c2", ...]
    const [t, headerStr, ...rest] = input;
    const splitCols = (str) => {
    ...
    throw new Error("Input must be a non-empty array or a { headers, rows } object");
  }
