...
const generateWAMessageContent = async (message, options) => {
    ...
    // WhatsApp requires every sticker to actually be WebP-encoded (not just named
    // ".webp") — verified via magic bytes rather than trusting the file extension.
    // We deliberately don't convert other formats here (that would require sharp or
    // a webp-capable jimp build); pass in stickers that are already WebP.
    ...
    if (!cover) {
        throw new boom_1.Boom('Sticker pack: a "cover" image (Buffer, { url } or { stream }) is required', { statusCode: 400 });
    }
    const stickerData = {};
    const stickerMetadata = await Promise.all(stickers.map(async (s, i) => {
        const stickerInput = s?.sticker ?? s;
        if (!stickerInput) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${i} is missing its media (expected { sticker: Buffer | { url } | { stream } })`, { statusCode: 400 });
        }
        const { stream } = await (0, messages_media_1.getStream)(stickerInput);
        const buffer = await (0, messages_media_1.toBuffer)(stream);
        if (!buffer || buffer.length === 0) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${i} is empty`, { statusCode: 400 });
        }
        if (!isWebPBuffer(buffer)) {
            throw new boom_1.Boom(`Sticker pack: sticker at index ${i} is not a valid WebP image. ` +
                'Convert it to WebP before passing it in (e.g. with sharp or an external converter).', { statusCode: 400 });
        }
        ...
        return {
            fileName,
            mimetype: 'image/webp',
            isAnimated: typeof s.isAnimated === 'boolean' ? s.isAnimated : isAnimatedWebP(buffer),
    ...
    const [stickerPackUpload, coverUpload] = await Promise.all([
        ...
        // prepareWAMessageMedia already resizes/re-encodes the cover to a JPEG via
        // sharp if installed, falling back to jimp automatically (same helper the
        // rest of this file already relies on for thumbnails/profile pictures) —
        // no extra image library needed here.
