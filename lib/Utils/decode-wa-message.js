...
/**
 * Extract addressing context from a binary node stanza.
 * Determines whether the message uses LID or PN addressing and extracts alt JIDs.
 */
function extractAddressingContext(stanza) {
    let senderAlt;
    let recipientAlt;
    const sender = stanza.attrs.participant || stanza.attrs.from;
    const addressingMode = stanza.attrs.addressing_mode || (sender && sender.endsWith('@lid') ? 'lid' : 'pn');
...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    const addrCtx = extractAddressingContext(stanza);
    const key = {
        remoteJid: chatId,
        remoteJidAlt: !(0, WABinary_1.isJidGroup)(chatId) ? addrCtx.senderAlt : undefined,
        ...
        participant,
        participantAlt: (0, WABinary_1.isJidGroup)(chatId) ? addrCtx.senderAlt : undefined,
        ...
        addressingMode: addrCtx.addressingMode,
