...
const getStream = async (item, opts) => {
    if (item === undefined || item === null) {
        throw new boom_1.Boom('Media input is missing — expected a Buffer, { stream }, or { url } but got ' + item, { statusCode: 400 });
    }
    if (Buffer.isBuffer(item)) {
        return { stream: (0, exports.toReadable)(item), type: 'buffer' };
    }
    if (typeof item === 'object' && 'stream' in item && item.stream) {
        return { stream: item.stream, type: 'readable' };
    }
    if (typeof item === 'object' && item.url) {
        return item.url.toString().startsWith('http://') || item.url.toString().startsWith('https://')
            ? { stream: await (0, exports.getHttpStream)(item.url, opts), type: 'remote' }
            : { stream: (0, fs_1.createReadStream)(item.url), type: 'file' };
    }
    throw new boom_1.Boom('Unsupported media input — expected a Buffer, { stream }, or { url } but got ' + JSON.stringify(item), { statusCode: 400 });
};
